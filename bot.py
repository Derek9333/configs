...
# Исправлен импорт ApplicationBuilder
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import (
    ApplicationBuilder, # Добавлен ApplicationBuilder
    Application, # Может быть удален, если ApplicationBuilder используется полностью
    CommandHandler,
    MessageHandler,
    filters,
    CallbackContext,
...
async def neural_detect_country(config: str) -> str:
    ...
    try:
        # Исправлена ошибка: добавлен await
...
async def button_handler(update: Update, context: CallbackContext) -> int:
    ...
    if query.data == 'add_file':
        ...
        return ConversationHandler.END
    # Возвращаем текущее состояние, если не обработано
    # Получаем текущее состояние из user_data или предполагаем WAITING_COUNTRY
...
def validate_config(config: str, target_country: str) -> tuple:
    ...
    try:
        ...
        if neural_client and len(config) < 500:
            # Исправлена ошибка: добавлен await
            neural_country = neural_detect_country(config) # Этот вызов должен быть асинхронным
...
def main() -> None:
    """Запуск бота"""
    # Исправлена ошибка: Убрано .updater(None) и добавлено правильное создание Application
    ...
    conv_handler = ConversationHandler(
        ...
        allow_reentry=True,
        per_user=True,
        per_chat=False, # Добавлено для совместимости с webhook
        per_message=False # Добавлено для совместимости с webhook
    )
    ...
    if external_host:
        ...
        # Настройка вебхука - теперь это должно работать
        application.run_webhook(
            listen="0.0.0.0",
            port=port,
            url_path="/webhook", # Добавлен url_path
